...
## UML Class Diagrams — Required Style

**This style is mandatory for all class diagrams.** Do NOT use flat rectangles with embedded text. Every class/trait/enum is a swimlane container with stackLayout children.

### Class container

```xml
<mxCell id="c100" value="ClassName" style="swimlane;fontStyle=1;align=center;verticalAlign=top;childLayout=stackLayout;horizontal=1;startSize=26;horizontalStack=0;resizeParent=1;resizeParentMax=0;collapsible=1;marginBottom=0;swimlaneHead=0;fillColor=default;" vertex="1" parent="1">
  <mxGeometry x="100" y="100" width="220" height="120" as="geometry"/>
</mxCell>
```

- `startSize=26` for plain classes/structs (single-line header)
- `startSize=41` for traits/enums with a stereotype (two-line header: `«Trait»\nName`)

### Stereotype header (traits and enums)

Set `value` on the container to double-escaped HTML:

```
&lt;div&gt;&amp;lt;&amp;lt; Trait &amp;gt;&amp;gt;&lt;/div&gt;TraitName
&lt;div&gt;&amp;lt;&amp;lt; Enum &amp;gt;&amp;gt;&lt;/div&gt;EnumName
```

This renders as `«Trait»` / `«Enum»` above the name in the header.

### Field/method rows

Each field or method is a **separate child `mxCell`** inside the container. `parent` = container id. `y` values accumulate from `startSize` (first child at `y=startSize`, each subsequent child at `y += previous_height`). Row height is **26** for normal rows.

```xml
<mxCell id="r100_1" value="+ field: Type" style="text;align=left;verticalAlign=top;spacingLeft=4;spacingRight=4;overflow=hidden;rotatable=0;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;whiteSpace=wrap;html=1;" vertex="1" parent="c100">
  <mxGeometry x="0" y="26" width="220" height="26" as="geometry"/>
</mxCell>
```

### Separator line

Between sections (fields / methods), use a separator row with `height=8`:

```xml
<mxCell id="r100_sep" value="" style="line;strokeWidth=1;fillColor=none;align=left;verticalAlign=middle;spacingTop=-1;spacingBottom=-1;spacingLeft=3;spacingRight=3;rotatable=0;labelPosition=right;points=[];portConstraint=eastwest;strokeColor=inherit;" vertex="1" parent="c100">
  <mxGeometry x="0" y="52" width="220" height="8" as="geometry"/>
</mxCell>
```

### Height calculation

Container `height` = `startSize` + sum of all child heights. Always calculate explicitly — draw.io does not auto-size.

### Encoding special characters in row values

| Raw | In XML attribute value |
|-----|----------------------|
| `<T>` (generic) | `&amp;lt;T&amp;gt;` |
| `->` (return arrow) | `-&gt;` |
| `&` | `&amp;` |
...
All edges use orthogonal routing with arc jump style:

```
edgeStyle=orthogonalEdgeStyle;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;exitX=0.5;exitY=1;exitDx=0;exitDy=0;entryX=0.5;entryY=0;entryDx=0;entryDy=0;jumpStyle=arc;
```

Arrow variants (append to base style):

| Relationship | Extra style |
|---|---|
| Trait impl (dashed open triangle) | `dashed=1;endArrow=block;endFill=0;` |
| Composition (filled diamond) | `startArrow=diamond;startFill=1;endArrow=none;endFill=0;` |
| Association (open arrow) | `endArrow=open;endFill=0;` |

Every edge `mxCell` must have a self-closing child:
```xml
...
drawio <filepath> &
```
Do NOT use `xdg-open` for `.drawio` files — use `drawio` directly.
